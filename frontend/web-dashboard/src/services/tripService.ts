...
export interface ExternalScreenshotUploadResponse {
  document_id: string;
...
export async function uploadExternalScreenshot(
  tripId: string,
  file: File,
  autoApply: boolean = true
): Promise<ApiResponse<ExternalScreenshotUploadResponse>> {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('auto_apply', String(autoApply));

  const res = await api.post<ApiResponse<ExternalScreenshotUploadResponse>>(
    `/trips/${tripId}/external-screenshot`,
    formData,
    {
      headers: {
