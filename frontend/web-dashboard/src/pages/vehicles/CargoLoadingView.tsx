        ...
        <div className="xl:col-span-9 flex flex-col justify-between gap-3.5 h-full overflow-hidden">
          ...
          <div className="w-full relative flex items-center justify-center shrink-0 overflow-hidden">
            ...
            <div className="relative w-full max-w-6xl xl:max-w-7xl mx-auto">
              ...
              {/* Framed Default Light HUD Overlay inside Trailer (Perfectly Centered & Aligned Margins) */}
              <div className="absolute top-[10.5%] left-[27.5%] w-[68.8%] h-[51.5%] bg-slate-100/90 backdrop-blur-sm text-slate-900 rounded-2xl p-2.5 sm:p-3 border border-slate-300/80 shadow-md flex items-stretch gap-2.5 sm:gap-3 overflow-hidden pointer-events-auto">
                
                {/* ── LEFT SUB-CARD: Odometer & Service Progress ── */}
                <div className="w-[34%] bg-white rounded-xl p-2.5 sm:p-3 flex flex-col justify-between border border-slate-200/90 shadow-2xs shrink-0 overflow-hidden">
                  
                  {/* Top Header */}
                  <div className="flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-6.5 h-6.5 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 shadow-2xs shrink-0">
                  ...
                  {/* Mechanical Rolling Counter Drum Wheels (Symmetrical & Centered) */}
                  <div className="my-auto py-1">
                    <div className="bg-slate-900 p-2 sm:p-2.5 rounded-xl border border-slate-800 shadow-inner flex items-center justify-center gap-1.5">
                      <div className="flex items-center gap-0.5">
                        {(() => {
                          ...
                          return digitArray.map((digit, idx) => (
                            <div
                              key={idx}
                              className="relative w-3.5 h-5.5 sm:w-4.5 sm:h-7 bg-gradient-to-b from-[#0A120E] via-[#17261F] to-[#0A120E] text-white font-mono font-black text-[11px] sm:text-xs rounded-md border border-slate-700 shadow-[inset_0_2px_4px_rgba(0,0,0,0.9)] flex items-center justify-center overflow-hidden shrink-0 select-none"
                            >
                      ...
                      <span className="text-[11px] sm:text-xs font-black text-slate-200 ml-1 shrink-0">
                        km
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50/90 border border-slate-200/90 rounded-xl p-2.5 space-y-1.5 shrink-0">
                    <div className="flex justify-between items-center text-[9px]">
                      <div className="flex items-center gap-1.5">
                        <div className="w-4 h-4 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-700 shrink-0">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] sm:text-xs font-black text-emerald-600 leading-tight mt-0.5">in 7,500 km</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-[8px] text-slate-400 font-extrabold uppercase leading-none">Target</p>
                        <p className="text-[11px] font-black text-slate-900 leading-tight mt-0.5">150,000 km</p>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center text-[8px] text-slate-400 font-semibold">
                ...
                {/* ── RIGHT SUB-CARD: Vehicle Service History (Pixel-Perfect Grid Column Alignment) ── */}
                <div className="flex-1 bg-white rounded-xl p-2.5 sm:p-3 flex flex-col justify-between min-w-0 border border-slate-200/90 shadow-2xs overflow-hidden">
                  ...
                  {/* 12-Column Header Bar (Identical Grid to Rows for Exact Column Alignment) */}
                  <div className="grid grid-cols-12 gap-2 bg-slate-100/90 px-3 py-1 rounded-lg text-[8.5px] font-extrabold text-slate-500 uppercase tracking-wider shrink-0 my-1 border border-slate-200/60 items-center">
                    <span className="col-span-6">Service</span>
                    <span className="col-span-3 text-center">Date</span>
                    <span className="col-span-3 text-right">Odometer</span>
                  </div>

                  {/* Service History Rows (12-Column Grid Matching Header) */}
                  <div className="flex-1 min-h-0 flex flex-col justify-around py-0.5 overflow-hidden gap-1.5">
                    {serviceRecords.slice(0, 3).map((rec, idx) => {
                      ...
                      return (
                        <div
                          ...
                          className="grid grid-cols-12 gap-2 items-center px-3 py-2 rounded-xl bg-slate-50/80 border border-slate-200/80 hover:border-slate-300 hover:bg-slate-100/80 transition-all cursor-pointer shrink-0"
                        >
                          {/* Service Work Done & Workshop (Col Span 6) */}
                          <div className="col-span-6 flex items-center gap-2.5 min-w-0">
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 shadow-2xs ${style.iconBg}`}>
                              <IconComponent className="w-3 h-3" />
                            </div>
                            <div className="min-w-0">
                              <p className="font-black text-slate-900 text-xs truncate leading-tight">{rec.work_done}</p>
                              <p className="text-[9px] font-medium text-slate-500 truncate mt-0.5">{rec.workshop_name}</p>
                            </div>
                          </div>

                          {/* Date (Col Span 3 - Centered) */}
                          <div className="col-span-3 text-center text-[10px] font-bold text-slate-600 whitespace-nowrap">
                            {rec.service_date}
                          </div>

                          {/* Odometer (Col Span 3 - Right Aligned) */}
                          <div className="col-span-3 text-right text-[11px] font-black text-slate-900 whitespace-nowrap">
