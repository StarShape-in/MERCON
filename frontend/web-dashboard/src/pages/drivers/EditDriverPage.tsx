  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 pb-8 space-y-4 animate-fade-in">
        ...
        {/* Instrument-Panel KPI Mini Cards Grid (MERCON Specification) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {/* Card 1: Operational Status */}
          <Card className="p-3.5 rounded-xl border border-border shadow-2xs bg-white dark:bg-card flex flex-col justify-between space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                OPERATIONAL STATUS
              </span>
              <div className="p-1 rounded-md bg-indigo-50 dark:bg-indigo-950 text-indigo-600">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">Quick update in status tab</p>
            </div>
          </Card>
          ...
          <Card className="p-3.5 rounded-xl border border-border shadow-2xs bg-white dark:bg-card flex flex-col justify-between space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="p-1 rounded-md bg-emerald-50 dark:bg-emerald-950 text-emerald-600">
                <IdCard className="w-3.5 h-3.5" />
              </div>
            </div>
            <div>
              <div className="text-xs font-mono font-bold text-foreground">
                {formData.license_number || 'No License #'}
              </div>
              <p className="text-[11px] font-medium text-emerald-600 dark:text-emerald-400 mt-0.5 flex items-center gap-1">
                {daysUntilExpiry !== null ? (
                  daysUntilExpiry < 0 ? (
                    <span className="text-rose-600 font-bold">Expired {Math.abs(daysUntilExpiry)} days ago</span>
                  ) : daysUntilExpiry < 30 ? (
                    <span className="text-amber-600 font-bold">Expires in {daysUntilExpiry} days</span>
                  ) : (
                    <span className="text-emerald-600 font-bold">Valid ({daysUntilExpiry} days left)</span>
                  )
                ) : (
                  'No expiry set'
                )}
          ...
          <Card className="p-3.5 rounded-xl border border-border shadow-2xs bg-white dark:bg-card flex flex-col justify-between space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="p-1 rounded-md bg-blue-50 dark:bg-blue-950 text-blue-600">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground truncate mt-0.5">
                {selectedVehicle ? `${selectedVehicle.asset_type}` : 'Click tab to assign'}
              </p>
            </div>
          </Card>

          {/* Card 4: AI Safety Rating */}
          <Card className="p-3.5 rounded-xl border border-border shadow-2xs bg-white dark:bg-card flex flex-col justify-between space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                SAFETY & RISK SCORE
              </span>
              <div className="p-1 rounded-md bg-purple-50 dark:bg-purple-950 text-purple-600">
                <ShieldCheck className="w-3.5 h-3.5" />
              </div>
            </div>
            <div>
              <div className="text-sm font-extrabold text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                {driver?.ai_risk_score ?? 98}% <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              </div>
              <p className="text-[11px] text-muted-foreground mt-0.5">Optimal Tier • Verified</p>
            </div>
          </Card>
        </div>
