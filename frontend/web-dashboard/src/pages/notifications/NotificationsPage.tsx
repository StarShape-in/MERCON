            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {(() => {
                ...
                for (const notif of filteredNotifications) {
                  ...
                  if (ds === todayStr) groups[0].items.push(notif);
                  else if (ds === yesterdayStr) groups[1].items.push(notif);
                  else if (d.getTime() >= weekAgoMs) groups[2].items.push(notif);
                  else groups[3].items.push(notif);
                }

                const groupLabelColors: Record<string, string> = {
                  Today: 'text-brand',
                  Yesterday: 'text-slate-600 dark:text-slate-300',
                  'This Week': 'text-slate-500 dark:text-slate-400',
                  Earlier: 'text-slate-400 dark:text-slate-500',
                };
                ...
                return groups
                  ...
                  .map((group) => (
                    <div key={group.label}>
                      ...
                      <div className="flex items-center gap-3 px-4 py-2 bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10">
                        <span className={`text-[10px] font-black uppercase tracking-widest ${groupLabelColors[group.label]}`}>
                          {group.label}
                        </span>
