...
export default function TripStepReview({
  ...
  return (
    <div className="space-y-3.5 animate-in fade-in-50 duration-200">
      ...
      <ReviewSection icon={<Receipt className="w-3.5 h-3.5 text-brand" />} title="Pricing" step={4} onEditStep={onEditStep}>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center pt-1">
          <div className="space-y-0.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-amber-500" /> Trip Rate
            </span>
            <span className="font-bold text-sm text-slate-900 dark:text-slate-100 block">
              {matchedRateCard ? `SAR ${matchedRateCard.base_price.toLocaleString()}` : total !== null ? `SAR ${total.toLocaleString()}` : 'Not set'}
            </span>
          </div>
          <div className="space-y-0.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 flex items-center gap-1">
              <Building2 className="w-3 h-3" /> Additional Charges
            </span>
            <span className="font-bold text-sm text-slate-500 dark:text-slate-400 block">SAR 0</span>
          </div>
          <div className="space-y-0.5 sm:text-right">
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Total Trip Amount</span>
            <span className="font-extrabold text-xl text-brand block">
              {total !== null ? `SAR ${total.toLocaleString()}` : 'Not set'}
            </span>
          </div>
        </div>
      </ReviewSection>
