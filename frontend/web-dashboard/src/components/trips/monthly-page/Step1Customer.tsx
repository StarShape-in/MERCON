...
export default function Step1Customer({
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 animate-fade-in py-1">
      <div className="p-4 bg-slate-50/70 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3.5 shadow-2xs">
            ...
            {customers.slice(0, 4).map((c, idx) => {
              ...
              return (
                <button
                  ...
                  className={`p-2.5 rounded-xl border text-left transition-all flex flex-col justify-between h-20 space-y-1 ${
                    isSelected
                ...
                >
                  <div className="flex items-center justify-between gap-1">
                    <span className={`w-6 h-6 rounded-lg font-bold text-[10px] grid place-items-center shrink-0 overflow-hidden ${
                    ...
                    <span className="text-[9px] font-bold text-slate-400 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-1 py-0.5 rounded shrink-0">
                      Key {idx + 1}
                    </span>
                  </div>
                  <div className="space-y-0">
                    <p className="text-xs font-bold text-slate-900 dark:text-slate-100 leading-snug truncate" title={c.name}>{c.name}</p>
                    <p className="text-[9px] text-slate-400 font-medium">Commercial Shipper</p>
                  </div>
                </button>
              );
            })}
        ...
        <div className="space-y-1.5 pt-2 border-t border-slate-200 dark:border-slate-800">
          ...
          {selectedCust && (
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-2 animate-fade-in mt-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-7 h-7 rounded-lg bg-orange-100 dark:bg-orange-950/60 text-[#E8450F] font-extrabold text-xs grid place-items-center shrink-0 border border-orange-200 dark:border-orange-800">
                    {initials}
                  </span>
                  <div>
                    <h5 className="text-xs font-bold text-slate-900 dark:text-slate-100">{selectedCust.name}</h5>
                    <p className="text-[10px] text-slate-500 font-medium">Commercial Account</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-xs">
                <div>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block flex items-center gap-1">
                    <Phone className="w-2.5 h-2.5 text-slate-400" /> CONTACT
                  </span>
                ...
                <div>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block flex items-center gap-1">
                    <CreditCard className="w-2.5 h-2.5 text-slate-400" /> TERMS
                  </span>
                  <span className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate block">
                ...
                <div>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block flex items-center gap-1">
                    <ShieldCheck className="w-2.5 h-2.5 text-slate-400" /> STATUS
                  </span>
