...
export const TripStep1Customer: React.FC<TripStep1CustomerProps> = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:col-span-6 space-y-2">
          ...
          {selectedCust ? (
            ...
            <div className="bg-orange-50/50 dark:bg-orange-950/20 rounded-xl border border-dashed border-orange-200/90 dark:border-orange-900/40 p-4 space-y-3 animate-fade-in shadow-2xs">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-orange-100 dark:bg-orange-900/60 text-brand flex items-center justify-center shrink-0 border border-orange-200">
                  <Building2 className="w-5 h-5 text-[#FA634E]" />
                </div>
                <div className="space-y-0.5">
                  <h5 className="text-xs font-extrabold text-[#3E3C3D] dark:text-slate-100 uppercase tracking-wider">
                    No Account Selected Yet
                  </h5>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    Search or select a customer on the left to lock commercial rate cards and load recent trips.
                  </p>
                </div>
              </div>

              {setIsCreateCustomerOpen && (
                <div className="pt-2 border-t border-orange-100 dark:border-orange-900/30 flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-slate-500">Need to create a new customer?</span>
                  <Button
                    type="button"
                    size="sm"
                    onClick={() => setIsCreateCustomerOpen(true)}
                    className="h-8 text-xs font-extrabold bg-[#FA634E] hover:bg-[#e04f3b] text-white rounded-lg gap-1.5 cursor-pointer shadow-2xs"
                  >
      ...
      {/* BOTTOM FULL-WIDTH SECTION: RECENT TRIPS (ALWAYS VISIBLE - ACCELERATOR FOR REPEAT TRIPS) */}
        ...
        <div className="flex items-center justify-between pb-1 border-b border-slate-100 dark:border-slate-800">
          <div>
            <h4 className="text-xs font-extrabold text-[#3E3C3D] dark:text-slate-100 uppercase tracking-wider flex items-center gap-1.5">
              ...
              {selectedCust ? `RECENT TRIPS (${selectedCust.name.toUpperCase()})` : 'RECENT TRIPS (ALL ACCOUNTS)'}
            </h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              {selectedCust
                ? `Repeat a previous trip configuration for ${selectedCust.name}`
                : 'Click "Repeat Trip" to auto-select customer and pre-fill route configuration'}
            </p>
          </div>
        </div>
        ...
        {isTripsLoading ? (
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900 shadow-2xs">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {displayRecentTrips.map((t) => {
                ...
                const custName = t.customer?.name || (customers.find((c) => c.id === t.customer_id)?.name) || '';
                  ...
                  >
                    ...
                    <div className="col-span-4 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {!selectedCust && custName && (
                          <span className="text-[10px] font-extrabold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 truncate max-w-[120px]">
                            {custName}
                          </span>
                        )}
                    ...
                    <div className="col-span-2 text-right">
                      <Button
                        ...
                        className="h-9 px-3 text-xs font-extrabold bg-[#FA634E] hover:bg-[#E0523D] text-white rounded-lg shadow-xs inline-flex items-center gap-1.5 cursor-pointer transition-all active:scale-[0.98]"
                      >
          ...
          <div className="bg-slate-50/60 dark:bg-slate-800/30 rounded-xl p-6 text-center space-y-1 border border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-400">
              Create a trip using the customer selection above.
            </p>
          </div>
        )}
      </div>
