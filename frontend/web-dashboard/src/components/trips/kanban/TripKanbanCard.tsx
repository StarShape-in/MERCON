...
export default function TripKanbanCard({
  ...
  return (
    ...
    >
      {/* ── 1. Top Bar: Ref ID + 3PL / Delayed Badges + Actions ───────────── */}
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col gap-1 min-w-0 flex-1">
          {/* Trip Reference ID & Badges */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="font-mono text-xs font-black text-slate-900 dark:text-slate-100 group-hover:text-brand transition-colors tracking-tight">
              {trip.ref_id}
            </span>

            {trip.is_third_party && (
              <Badge className="bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300 border-purple-200 dark:border-purple-800 font-bold text-[10px] px-1.5 py-0 rounded-md">
                3PL
              </Badge>
            )}
      ...
      {/* ── 2. Route Hero Module: High-clarity visual route pathway ──────── */}
      <div className="bg-slate-50/90 dark:bg-slate-800/60 rounded-xl p-2.5 flex flex-col gap-1.5 border border-slate-200/80 dark:border-slate-700/60 shadow-3xs">
        {/* Origin (Pickup) */}
        <div className="flex items-center gap-2 min-w-0">
          <div
            className="w-2 h-2 rounded-full bg-emerald-500 shrink-0 ring-2 ring-emerald-100 dark:ring-emerald-950 shadow-2xs"
            title="Pickup Origin"
          />
        ...
        <div className="flex items-center gap-2 pl-0.5 text-slate-400 dark:text-slate-500">
          <div className="w-1 h-3 border-l-2 border-dashed border-slate-300 dark:border-slate-600 ml-0.5" />
        ...
        <div className="flex items-center gap-2 min-w-0">
          <div
            className="w-2 h-2 rounded-full bg-rose-500 shrink-0 ring-2 ring-rose-100 dark:ring-rose-950 shadow-2xs"
      ...
      {/* ── 3. Assignment Module: Driver & Vehicle ────────────────────────── */}
      <div className="flex items-center justify-between gap-2 text-xs pt-0.5">
        {/* Driver */}
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          {trip.is_third_party ? (
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-5 h-5 rounded-full bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-300 font-bold text-[9px] flex items-center justify-center shrink-0 border border-purple-200 dark:border-purple-800">
                3P
              </div>
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-[9px] flex items-center justify-center shrink-0 border border-slate-200/80 dark:border-slate-700">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <Truck size={13} className={trip.is_third_party ? 'text-purple-500' : 'text-slate-400'} />
          <span className="font-mono text-[11px] font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-md border border-slate-200/60 dark:border-slate-700/60 truncate max-w-[100px]">
            {trip.is_third_party
      ...
      {/* ── 4. Card Footer: Metadata badges + Financial Amount ────────────── */}
      <div className="pt-2.5 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 flex-wrap min-w-0">
          ...
          {rateCat !== '—' && (
            <Badge
              variant="outline"
              className="text-[10px] py-0 px-1.5 font-bold bg-indigo-50/60 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border-indigo-200/80 rounded-md"
            >
              {rateCat}
            </Badge>
          )}
        ...
        <div className="font-mono text-xs font-black text-slate-900 dark:text-slate-100 shrink-0">
