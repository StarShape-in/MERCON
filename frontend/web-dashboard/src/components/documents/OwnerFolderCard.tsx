...
export default function OwnerFolderCard({ row, onOpen, onUploadMissing }: OwnerFolderCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* ── RIGHT COMPLIANCE CONTENT PANEL (SPACIOUS & UNCLIPPED) ── */}
      <div className="flex-1 p-6 flex flex-col justify-between space-y-5 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <button
            ...
            className="h-9 px-4 text-xs font-black text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-900 rounded-xl flex items-center gap-2 hover:bg-indigo-100 dark:hover:bg-indigo-900/80 transition-all shadow-3xs cursor-pointer shrink-0 whitespace-nowrap self-start sm:self-center"
          >
        ...
        {/* Spacious Document Cards List (Zero Text Collisions or Character Wrapping) */}
          ...
          {mandatorySlots.map((slot) => {
            ...
            return (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-2xl border border-slate-200/80 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all cursor-pointer bg-slate-50/50 dark:bg-slate-850/40"
              >
                {/* Left: Standalone Icon + Full Name */}
                ...
                {/* Right: Status Badge & Isolated Date */}
                <div className="flex items-center gap-2 shrink-0">
                  <span
                    className={cn(
                      "text-[10px] font-black px-2 py-0.5 rounded-md font-mono shrink-0",
                      isValid
                  ...
                  <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500 min-w-[65px] text-right shrink-0">
                    {formattedDate || '—'}
                  </span>
                </div>
              </div>
