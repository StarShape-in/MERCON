...
export const updateCustomerSavedLocation = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { customerId, label, address, lat, lng } = req.body;
    const userId = getValidUuid((req as any).user?.id);
    ...
    const updated = await prisma.customerSavedLocation.update({
      where: { id: id as string },
      data: {
        customerId: normalisedCustomerId,
        label: trimmedLabel,
    ...
    res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to update saved location' } });
  }
};
